...
export class UserService {
    ...
    async getOneByEmail(email: string): Promise<IUser> {
        ...
        if (!found) {
            throw new NotFoundException(`User with email '${email}' could not not found`);
        }
        return found;
    }
    ...
    async create(newUser: InsertUser) {
        ...
        return await this.database.insert(user).values({email, password: hashedPassword}).returning({userId: user.id, email: user.email});
    }
}
