...
export class DbAccessService {
  ...
  patchCompetitionReviewData(reviewData: ICompReviewUpdate) {
    const reply = this.http.patch<string>(
      'api/competitions/update-comp',
      reviewData
    );
    console.log(reply);
    return reply;
  ...
  getFinalisedCompetitionPlayersDetails(compId: number) {
    return this.http.get<IResultPlayers[]>(
      `api/competitions/results/${compId}`
    );
  }

  getTeesPlayedInCompetition(compId: number) {
    return this.http.get<ITeesFromCompetitionID[]>(
      `api/tees/competition/${compId}`
    );
  }
